...
def add_color(tweets):
    ...
    for t in tweets:
        score = t['score']
        color_gradient = int((round(score/0.02) * 0.02 + 1)*49.5)
        t['color'] = colors[color_gradient]
...
def tweets(name):
    ...
    tweets_record = fetch_tweets(api,name)
    add_color(tweets_record['tweets'])
    color_total = []
    for tweet in tweets_record['tweets']:
        color_total.append(tweet['score'])
    senti_median = median(color_total)
    return render_template('tweets.html', record = tweets_record, median = senti_median)
...
def following(name):
    ...
    return render_template('following.html', name = name, followers = followers)
